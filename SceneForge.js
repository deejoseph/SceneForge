...
function confirmFinishList() {
  ...
  // 设置测试图片路径（关联瓷韵初稿）
  formData.fused_product_path = `http://127.0.0.1:8888/${selectedProductCategory.code}/${productList[0].product.file}`;
...
function setChoice(key, id, label, nextId) {
  console.log("setChoice 调用:", key, id, label, nextId);
  ...
  if (key === 'mood') {
    ...
    if (genBtn) {
      ...
      genBtn.removeAttribute('disabled');  // 确保移除 disabled 属性
  ...
  if (key === 'purpose') {
    ...
    const pkgSection = document.getElementById("packagingSection");
    if (pkgSection) {
      pkgSection.style.display = id === 'gift' ? "block" : "none";
    }
  }
